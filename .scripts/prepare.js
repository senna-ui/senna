  ...
  const prompts = [
    {
      ...
      filter: (input) =>
        isValidVersionInput(input) ? getNewVersion(oldVersion, input) : input,
    },
    {
      type: "input",
      name: "version",
      ...
      filter: (input) =>
        isValidVersionInput(input) ? getNewVersion(pkg.version, input) : input,
      validate: (input) => {
...
function updateCoreReadme(tasks, version) {
  tasks.push({
    ...
    task: () =>
      execa("node", ["update-readme.js", version], {
        cwd: path.join(common.rootDir, "core", "scripts"),
      }),
  });
}
...
const isValidVersionInput = (input) =>
  SEMVER_INCREMENTS.indexOf(input) !== -1 || common.isValidVersion(input);
