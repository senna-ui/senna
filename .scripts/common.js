  ...
  const prompts = [
    {
      ...
      choices: ["latest", "next", "v4-lts"].concat([
...
function updatePackageVersions(tasks, packages, version) {
  packages.forEach((package) => {
    ...
    tasks.push({
      title: `${package} update @senna-ui/core dependency, if present ${dim(
        `(${version})`
      )}`,
      task: async () => {
        if (package !== "core") {
    ...
    if (package === "packages/react-router") {
      tasks.push({
        title: `${package} update @senna-ui/react dependency, if present ${dim(
          `(${version})`
        )}`,
        task: async () => {
          const pkg = readPkg(package);
...
function copyCDNLoader(tasks, version) {
  tasks.push({
    ...
    task: () =>
      execa("node", ["copy-cdn-loader.js", version], {
        cwd: path.join(rootDir, "core", "scripts"),
      }),
  });
}
